...
from .n_sigma_deployer import NSigmaDeployer
from ..offline_job import OfflineJobLauncher
from ..collector import MyDataCollector, MyPromCollector
from scheduler.baselines import NSigmaScheduler, N_SIGMA_NAME
...
def start_experiment_handler():
    ...
    scheduler = NSigmaScheduler(cluster, "data/understanding_11/node_data.csv")
    scheduler.run()
    manager.components.set("scheduler", scheduler)
    base_deployer = NSigmaDeployer(
    ...
    offline_job_launcher = OfflineJobLauncher(
        configs_obj.file_paths["offline_job_output_path"], N_SIGMA_NAME
    )
...
def end_experiment_handler():
    ...
    assert isinstance(scheduler, NSigmaScheduler)
    scheduler.stop()
...
def start_workload_handler():
    ...
    assert isinstance(configs_obj, configs.Configs)
    assert isinstance(test_case, TestCase)
    assert isinstance(deployer, NSigmaDeployer)
