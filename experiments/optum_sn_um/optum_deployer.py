...
class OptumDeployer(BaseDeployer):
    def __init__(
        self,
        namespace: str,
        pod_spec: PodSpec,
    ...
    def prepare_under_test_yaml(
        ...
        ).assign_affinity(nodes=self.testbed_nodes).assign_containers(
            replicas=replicas
        ).update(
            "spec.template.spec.containers[0].imagePullPolicy", "IfNotPresent"
        ).update("spec.template.spec.schedulerName", SCHEDULER_NAME).update(
